...
///*
//Thinking process:
//Very similar to Majority I, II,
// except we can use a HashMap to store information (value, count).
//Having a HashMap we have one advantage:
// when checking if current value matches any
// previously recorded val, just do a map.containsKey(val).
//When a pair in hashMap has
// count ==0, remove this pair.
//Note, to learn how to use iterator in HashMap.

//Note: when map.containsKey(currVal) == false,
// the code checks map.size() == k
// before count-- perations. This is because:
//We first need to put k candidates into HashMap
// before we count-- from all of them.
// If map.size() < k, that means we still have
// free spot for candidate in the HashMap,
// so in this case we do: map.put(candidateKey, 1).
//
//*/
public class MajorityElement3 {
