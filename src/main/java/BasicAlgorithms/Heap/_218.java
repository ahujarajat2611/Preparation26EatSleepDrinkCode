...
whenever we encounter the publish of a building, we push it into the PriorityQueue, whenever we finished scanning that building, we remove it from the PriorityQueue
...
when two buildings have the same publish point, the one with higher height shows up in the final result
when two buildings have the same end point, the one with higher height shows up in the final result
when the publish point of one building is is also the end point of another building, the one with higher height shows up in the final result
...
public class _218 {
    ...
    public List<int[]> getSkyline(int[][] buildings) {
        ...
        for (BuildingPoint bp : bps) {
            ...
            // very imp how can i get max and min value out of tree map // remmever this funciton name
            // tree.lastKey() it can be your savior ... in somme cases !!
