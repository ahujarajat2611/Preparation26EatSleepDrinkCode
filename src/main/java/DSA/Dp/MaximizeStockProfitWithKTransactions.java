...
public class MaximizeStockProfitWithKTransactions {
    ...
    public int maxProfitWithAtMost2Transactions(int[] a, int n) {
        ...
        ConsoleWriter.printArray(profit);
        int min_so_far = a[0];
        for (int i = 1; i < n; i++) {
    ...
    public int maxProfitWithKTransactions(int[] a, int n, int k) {
        ...
        // Always think about the end state you can reac and iterate from all possible states before that
        // thats the mantra of DP simple as hell but quite difficult to apply !!!!
        // end state gola always keep that in mind !!!!!!!!!
    ...
    public int maxProfitWithKTransactionsWithMaxDiff(int[] a, int n, int k) {
        ...
        for (int i = 1; i <= k; i++) {
            ...
            for (int j = 1; j < n; j++) {
                // sell second element
                // max money you can have in your pocket with selling jth elemtn
                t[i][j] = Math.max(t[i][j - 1], maxDiff + a[j]);
                // buy second element
                // see which element you can buy with maximum money left in the pocket
                //
