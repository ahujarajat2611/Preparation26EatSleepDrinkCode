...
public class SplitArrayLargestSum {
	...
	public int splitArray(int[] a, int m) {
		...
		long l = max, r = sum;
		// if number of partitions less than 1
		// then whole array sum will be ans
		if (m <= 1)
			return sum;
		// if number of partitions
		else if (m >= a.length) {
			return max;// binary search
		}
		else {
			while (l < r) {
				//	while (l <= r) {
				...
				// if mid is valid then try reducing and see how much you can reduce !!!!!further !!!
				if (valid(a, mid, m)) {
					r = mid;
					//	r = mid-1 ;
				} else {
