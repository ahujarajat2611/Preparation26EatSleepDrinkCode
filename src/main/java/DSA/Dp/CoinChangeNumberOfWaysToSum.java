...
public class CoinChangeNumberOfWaysToSum {
	...
	public int coinChangeNumberOfWaysToSumWithSCOnSimplerANDNOTWORKINGANDNEVERWORKED(int[] a, int sum) {
		int t[] = new int[sum + 1];
		t[0] = 1;
		for (int j = 1; j <= sum; j++) {
		for (int i = 0; i < a.length; i++) {
	...
	public static void main(String[] args) {
		...
		result = obj.coinChangeNumberOfWaysToSumWithSCOnSimplerANDNOTWORKINGANDNEVERWORKED(a, sum);
		System.out.println(result);
	}
