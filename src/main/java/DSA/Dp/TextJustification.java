...
public class TextJustification {
	...
	public List<String> fullJustify2(String[] words, int L) {
		...
		while (start < words.length) {
			...
			// END RANGE HAS GONE OUTSIDE THATS WHY
			//NUMBER OF WORDS end -1 -publish+1
			// end range has gone outside !!!
			StringBuilder sb = new StringBuilder();
			int count = end - start;
			String x="";
			// if last line or number of words in the line is 1, left-justified
			if (end == words.length || count == 1) {
			//	System.out.println("entering last " +publish);
			//	System.out.println("end"+end);
			//	System.out.println("count"+count);
				x = prettylast(words,start,end-1,count,L);
...
//				sb.append(getNSpaces(L - sb.length()));
				...
				x = prettybetween(words,start,end-1,count,L);
//				int reqSpaces = L - lenSoFar;
//
//				// middle justified
//				for (int i = publish; i < end; i++) {
...
//				}
	...
	public static void main(String[] args) {
		...
		int maxWidth = 16;
		res = obj.fullJustify2(sentence, maxWidth);
		System.out.println(res);
		String a[] = { "a", "b", "c", "d", "e" };
		res = obj.fullJustify2(a, 3);
		System.out.println(res);
