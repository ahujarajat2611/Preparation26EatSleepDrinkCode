...
public class AllSubarraySumToK {
	 ...
	 * Given an array nums and a target value k,
	  * find the maximum length of a subarray
	   * that sums to k. If there isn't one,
	    * return 0 instead.
 ...
 */
    // Time : O(n), Space : O(n)
    // AB tak sum ka count kya hai so taht if that comes again i can see quickly in my hashmap ... thatst he trick to keep
    // thinsg in state
 ...
 */
    ...
    public boolean checkSubarraySum_MultipleOfK(int[] a, int k) {
        ...
        for (int i = 0; i < a.length; i++) {
            ...
            if (k != 0) {
                // update sum to take modulous and begin your thing
                //
                sum %= k;
            }
            // map contains key !!!!!
            //
            if (map.containsKey(sum)) {
                // why this more than Sum not sure !!! why is that the case length of subarray current index - map.get(sum)
                // taht is the answer !!!
                // currentindex -map.get(sum ) will get you length of subarray meeting reuqired ocndition d
                if (i - map.get(sum) > 1) {
                    return true;
                }
            } else {
...
Explanation: There is one 0-diff pair in the array, (1, 1).
    ...
    public int findPairs(int[] a, int k) {
        ...
        for (Map.Entry<Integer, Integer> e : map.entrySet()) {
            if (k == 0) {
                // we need to have duplicates to take care of k ==0 case else not required thats why hashmapo
                if (e.getValue() >= 2) {
                    count++;
                }
            } else {
                if (map.containsKey(e.getKey() + k)) {
                    /// since we have to find unique so that just counting that number straight forward !!!!!!
                    count++;
                }
            }
        }
