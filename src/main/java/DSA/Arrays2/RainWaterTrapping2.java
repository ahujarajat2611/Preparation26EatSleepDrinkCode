		...
		while (!queue.isEmpty()) {
			...
			for (int[] dir : dirs) {
				...
				if (row >= 0 && row < m && col >= 0 && col < n && !visited[row][col]) {
					...
					// if you understand previous problem // ten yoou need to pass on max height not 
					queue.offer(new Cell(row, col, Math.max(heights[row][col], Math.max(cell.height,heights[row][col]))));
				}
			}
		}
