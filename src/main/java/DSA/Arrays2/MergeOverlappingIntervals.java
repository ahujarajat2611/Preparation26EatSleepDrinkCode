		...
		public List<Interval> mergeOverlappingIntervals(List<Interval> a, int n) {
			...
			for (int i = 1; i < a.size(); i++) {
				...
				if (cur.start <= prev.end) {
					// min part actually not required since prev would hav started earlier for sure
					merged = new Interval(Math.min(cur.start,prev.start), Math.max(cur.end, prev.end));
					prev = merged;
					// also here are creating new list ( new answer list so no same list update chakkar 
					// very nicely done !!!
				} else {
					result.add(prev);
					prev = cur;
