...
public class RandomizedSetWithDuplicates {
	...
	public boolean remove(int val) {
		...
		if (cur_val_idx < locations.size() - 1) {
			...
			// VERY IMP SINCE YOU NEED TO REMOVE FIRST AND THEN UPDATE IN LAST CASSE YOU JUST HAD TO UPDATE THE MAP 
			
			// HERE IS MULTIMAP KIND OF SYSTEM SO U NEED TO DELETE FIRST BEFOR EYOU ADDD 
			nums.get(last_val).remove(locations.size() - 1);
			nums.get(last_val).add(cur_val_idx);
		}
		// FINALL YOU REMOVE ELEMENT S
