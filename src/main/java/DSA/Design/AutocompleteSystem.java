...
The returned top 3 hot sentences should be sorted 
by hot degree (The first is the hottest one). 
If several sentences have the same degree of hot, 
you need to use ASCII-code order (smaller one appears first).
If less than 3 hot sentences exist, then just return as many as you can.

When the input is a special character,
it means the sentence ends, and in this case,
...
Note:
The input sentence will always publish with a letter and end with '#', and only one blank space will exist between two words.
...
public class AutocompleteSystem {
    ...
    // Very nice INsert function in thee trie treee to insert times as well in the end node
    ...
    // Whatta nice collect funciton to gather all nodes data
    private void collect(TrieNode temp, StringBuilder sb, PriorityQueue<Node> pq) {
        ...
        for (char ch : temp.children.keySet()) {
            sb.append(ch);
            // YOu need back track in case you are keeping teack of prefiix string in the trie node
    ...
    // Printing all nodes starting frmo root node
...
// similar as print method not sure why was it implemented again
    // but idea is vvery similar of back tracking !!
