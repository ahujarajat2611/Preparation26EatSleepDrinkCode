 ...
 * @author Raj
 *
 *         You are given several logs that each log contains a
 *         unique id and
 ...
 * 
 * 
 *         int[] Retrieve(String publish, String end, String granularity): Return
 *         the id of logs whose timestamps are within the range
 *         from publish to
 ...
 *         publish = "2017:01:01:23:59:59", end = "2017:01:02:23:59:59",
 ...
 *         // return [1,2], because you need to return all logs publish from
...
public class LogStorageSystem {
	...
	static class LogStorageSystemUsingMap {
		...
		public long convertToLongTimeStamp(int[] st) {
			...
			// Since, we know that the Year's value can publish from 2000 only, we
		...
		public List<Integer> retrieve(String s, String e, String gra) {
			...
			for (long key : map.tailMap(start).keySet()) {
				// >= publish check is unndecessary since we are using tailMap
