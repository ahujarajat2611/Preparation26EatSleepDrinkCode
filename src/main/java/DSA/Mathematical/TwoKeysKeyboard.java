 ...
 * @author Raj
 * 
 * Initially on a notepad only one character
 * 'A' is present.
...
Given a number n. You have to get exactly n 'A'
on the notepad by performing the minimum number of steps permitted.
...
public class TwoKeysKeyboard {
    ...
    public int minStepsMine(int n) {
        ...
        for(int end =2 ;end<=n;end++){
            ...
            for(int i=1;i<end;i++){
                if(end%i ==0){
                    dp[end] = Math.min(dp[end],dp[i]+(end/i)  );
                }
            }
        }
        return dp[n];
