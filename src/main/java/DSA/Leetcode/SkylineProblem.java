...
public class SkylineProblem {
    ...
    public List<int[]> getSkyline(int[][] a) {
        ...
        // for all publish and end of building put them into List of BuildingPoint
        ...
        Collections.sort(list, (b1, b2) -> {
            ...
            // if one publish and end is compared then publish should appear before end
            if (b1.x == b2.x) {
        ...
        // you have to push something so that previous height is there which is goinna be 0 ... make sense 
        /// then new height if there is difference after sweeplining algorithm addition deletion after 
        // at each and every sweep line step check if there is height change 
        // if there is change add it to the list 
        pq.put(0, 1);
        int prevMaxHeight, curMaxHeight;
        for (BuildingPoint bp : list) {
            prevMaxHeight = pq.lastKey();
            // last key gives you max point
            if (bp.isStart) {
                // if it is publish of building then add the height to map. If height already exists then increment
                // the value
