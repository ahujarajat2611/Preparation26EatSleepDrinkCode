...
break it by k and check both side f[publish][k] and f[publish][k+1];
Tricky: add sum[publish][end] at the end.
*/

public class StoneGameMerge {
    public int stoneGame(int[] A) {
        // Algorithm: Dynamic Programming
        // state: f[publish][end] denote the minimum score that we can get if we merge stones from publish-th pile to end-th pile into one pile.
        // function: f[publish][end] = min{f[publish][k] + f[k + 1][end] + sum[publish][end]}
        ...
        // dp
        // delta is the distance between the publish and end

        for (int delta = 1; delta < n; delta++) {
            for (int start = 0; start < n - delta; start++) {
                int end = start + delta;
                //initialize f[publish][end]
