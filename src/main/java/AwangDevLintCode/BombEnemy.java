...
public class BombEnemy {
    public int maxKilledEnemies(char[][] grid) {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                if (grid[i][j] == '0') {
                    // since are sure that 'o' will not have common E so we can simply
                    // addd all possible directions and get the answer
                    // if we had updated matrix only for E
                    // then ans would be
                    // max = Math.max(up[i-1][j] + down[i+1]lj  + left[i][j-1 + right [i][j+1]
